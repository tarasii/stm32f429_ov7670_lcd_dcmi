...
uint8_t DCMI_SingleRandomWrite(uint8_t Reg, uint8_t Data)
{
	...
	buff[0] = Reg;
	buff[1] = Data;	
	if (I2C_Write(Open407V_SCCB, buff, 2, OV7670_DEVICE_WRITE_ADDRESS) == Error) return 0xFF;
	return 0;
}
...
{
	if (I2C_Write(Open407V_SCCB, &Reg, 1, OV7670_DEVICE_READ_ADDRESS) == Error) return 0xFF;
	if (I2C_Read(Open407V_SCCB, Data, 1, OV7670_DEVICE_READ_ADDRESS) == Error) return 0xFF;
	return 0;
}
